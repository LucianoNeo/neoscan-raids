  ...
  return (
    <div className="relative md:rounded-lg rounded-3xl overflow-hidden w-[120vw] md:w-64">
      ...
      <div className="justify-between md:w-60 absolute z-10 text-white top-4 text-7xl md:text-3xl font-extrabold left-4 ">
        ...
        {props.level == 7 && <h1>MEGA</h1>}
        {props.level == 10 && <h1>PRIMAL</h1>}
        <h1 className="drop-shadow-lg truncate">{props.name.toUpperCase()}</h1>
        <h1 className="flex items-center gap-2">
          {props.level} <Star weight="fill" color="yellow" />
        </h1>
      </div>
      ...
      <div className="w-full pt-16 pb-4 px-4 bg-game-gradient absolute bottom-0 left-0 right-0 items-center justify-center">
        <img
          ...
          alt=""
          className="w-[600px] md:w-36 absolute md:bottom-28 md:left-14 bottom-64 left-1"
        />
        <div className="flex flex-col gap-2 md:gap-0">
          <span className="text-white block text-4xl md:text-sm  z-10">
            Ginásio:
          </span>
          <p className="text-white font-bold truncate text-4xl md:text-sm">
            {props.title}
          </p>
          <span className="text-white block text-4xl md:text-sm">Começou:</span>
          <strong className=" block text-4xl md:text-sm text-blue-500">
            {props.start}
          </strong>
          <span className="text-white block text-4xl md:text-sm">Termina:</span>
          <strong className=" block text-4xl md:text-sm text-red-700">
            {props.end}
          </strong>
        </div>
      </div>
