...
export default function handler(req, res) {
    mysql.getConnection((error, conn) => {
        ...
        conn.query(
            'select * from raid left join gymdetails on raid.gym_id = gymdetails.gym_id left join gym on raid.gym_id = gym.gym_id WHERE END >= NOW() + INTERVAL 185 MINUTE && START <= NOW() + INTERVAL 185 MINUTE && pokemon_id IS NOT NULL ORDER BY END DESC LIMIT 500 ;',
